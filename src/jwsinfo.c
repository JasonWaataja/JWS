...
{
	JwsInfoPrivate *priv = jws_info_get_instance_private(info);
	return g_strdup(priv->background_color);
}
...
{
	JwsInfoPrivate *priv = jws_info_get_instance_private(info);
	priv->background_color = g_strdup(background_color);
}
...
jws_info_set_time_from_line(JwsInfo *info, const gchar *line, GError **err)
{
	...
	if (!found_match) {
		g_set_error(err, JWS_INFO_ERROR, JWS_INFO_ERROR_FILE_FORMAT,
			"No argument was found to time in line: \"%s\".", line);
		g_regex_unref(regex);
	...
	if (!value || strlen(value) == 0) {
		g_set_error(err, JWS_INFO_ERROR, JWS_INFO_ERROR_FILE_FORMAT,
			"Failed to find valid argument to time.");
		g_regex_unref(regex);
		g_match_info_free(match_info);
		g_free(value);
		return FALSE;
	...
	if (!rotate_time) {
		g_set_error(err, JWS_INFO_ERROR, JWS_INFO_ERROR_FILE_FORMAT,
			"Failed to parse time sting: \"%s\".", value);
		g_regex_unref(regex);
		g_match_info_free(match_info);
		g_free(value);
		return FALSE;
...
jws_info_set_mode_from_line(JwsInfo *info, const gchar *line, GError **err)
{
	...
	if (!value || strlen(value) == 0) {
		g_set_error(err, JWS_INFO_ERROR, JWS_INFO_ERROR_FILE_FORMAT,
			"Couldn't find argument for mode.");
		g_regex_unref(reg);
		g_match_info_free(match_info);
		g_free(value);
		return FALSE;
...
static bool
jws_info_set_background_color_from_line(JwsInfo *info, const gchar *line,
	GError **err)
{
	// Match a line starting with background-color, then an argument;
	// capture the argument.
	GRegex *reg = g_regex_new("^background-color\\s+(\\S+)$", 0, 0, NULL);
	g_assert(reg);
	GMatchInfo *match_info = NULL;
	gboolean found_match = g_regex_match(reg, line, 0, &match_info);
	if (!found_match) {
	...
	if (!value || strlen(value) == 0) {
		g_set_error(err, JWS_INFO_ERROR, JWS_INFO_ERROR_FILE_FORMAT,
			"Couldn't find argument for background-color.");
		g_regex_unref(reg);
		g_match_info_free(match_info);
		g_free(value);
		return FALSE;
	...
	if (strlen(value) != 7 || value[0] != '#') {
		g_set_error(err, JWS_INFO_ERROR, JWS_INFO_ERROR_FILE_FORMAT,
			"Not a hex color \"%s\".", value);
		g_regex_unref(reg);
		g_match_info_free(match_info);
		g_free(value);
		return FALSE;
	}
	jws_info_set_background_color(info, value);
	g_regex_unref(reg);
	g_match_info_free(match_info);
	g_free(value);
	return true;
	...
	GError **err)
{
	...
	} else if (g_str_has_prefix(line, "background-color")) {
		GError *tmp_err = NULL;
		if (!jws_info_set_background_color_from_line(info, line,
			    &tmp_err)) {
			g_propagate_error(err, tmp_err);
			return FALSE;
		}
...
print_jws_info(JwsInfo *info)
{
	...
	if (priv->rotate_image) {
		g_print("Rotate image\n");
		g_print("Seconds between rotation: %i\n",
			jws_time_value_total_seconds(priv->rotate_time));
		if (priv->randomize_order)
			g_print("Randomize order\n");
		else
			g_print("In order");

	} else
		g_print("Single image\n");
