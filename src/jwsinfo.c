...
static void jws_info_dispose(GObject *obj) {
        G_OBJECT_CLASS(jws_info_parent_class)->dispose(obj);
}

static void jws_info_finalize(GObject *obj) {
...
static void jws_info_init(JwsInfo *self) {
...
static void jws_info_class_init(JwsInfoClass *kclass) {
...
JwsInfo *jws_info_new() { return g_object_new(JWS_TYPE_INFO, NULL); }

JwsInfo *jws_info_new_from_file(const gchar *path, GError **err) {
...
gboolean jws_info_get_rotate_image(JwsInfo *info) {
        g_assert(info);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
        return priv->rotate_image;
}

void jws_info_set_rotate_image(JwsInfo *info, gboolean rotate_image) {
        g_assert(info);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
        priv->rotate_image = rotate_image;
}

JwsTimeValue *jws_info_get_rotate_time(JwsInfo *info) {
        g_assert(info);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
        return jws_time_value_copy(priv->rotate_time);
}

void jws_info_set_rotate_time(JwsInfo *info, JwsTimeValue *time) {
        g_assert(info);
        g_assert(time);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
        jws_time_value_free(priv->rotate_time);
...
gboolean jws_info_get_randomize_order(JwsInfo *info) {
        g_assert(info);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
        return priv->randomize_order;
}

void jws_info_set_randomize_order(JwsInfo *info, gboolean randomize_order) {
        g_assert(info);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
        priv->randomize_order = randomize_order;
}

GList *jws_info_get_file_list(JwsInfo *info) {
        g_assert(info);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
        return priv->file_list;
}

void jws_info_set_file_list(JwsInfo *info, GList *file_list) {
        g_assert(info);
        g_assert(file_list);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
...
void jws_info_add_file(JwsInfo *info, const gchar *path) {
        g_assert(info);
        g_assert(path);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
...
gboolean jws_info_remove_file(JwsInfo *info, const gchar *path) {
        g_assert(info);
        g_assert(path);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
        ...
        while (current) {
                ...
                if (g_str_equal(current_path, path)) {
                        priv->file_list =
                            g_list_delete_link(priv->file_list, current);
                        return TRUE;
                }
        }
        return FALSE;
 ...
 */
static gboolean jws_info_set_time_from_line(JwsInfo *info, const gchar *line,
                                            GError **err) {
        g_assert(info);
        g_assert(line);
        GRegex *regex;
 ...
 */
static gboolean jws_info_set_mode_from_line(JwsInfo *info, const gchar *line,
                                            GError **err) {
        g_assert(info);
        g_assert(line);
        /*
 ...
 */
static gboolean jws_info_process_line(JwsInfo *info, const gchar *line,
                                      gboolean *has_files, GError **err) {
        g_assert(info);
        g_assert(line);
        g_assert(has_files);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
...
gboolean jws_info_set_from_file(JwsInfo *info, const gchar *path,
                                GError **err) {
        g_assert(info);
        g_assert(path);
        ...
        GIOStatus status = g_io_channel_read_line(channel, &line, &line_length,
                                                  &terminator_pos, NULL);
        while (status == G_IO_STATUS_NORMAL) {
                if (line && terminator_pos) line[terminator_pos] = '\0';
...
void print_jws_info(JwsInfo *info) {
        g_assert(info);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
 ...
 */
static gboolean jws_info_write_rotate_image(JwsInfo *info, GIOChannel *writer) {
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
        if (!priv->rotate_image) return jws_write_line(writer, "single-image");
        ...
        gchar *time_str =
            g_strdup_printf("time %ih%im%is", simplest_form->hours,
                            simplest_form->minutes, simplest_form->seconds);
...
gboolean jws_info_write_to_file(JwsInfo *info, const gchar *path) {
        g_assert(info);
        g_assert(path);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
...
gboolean jws_write_line(GIOChannel *channel, const gchar *message) {
        gchar *new_message = g_strconcat(message, "\n", NULL);
        gsize size;
        GIOStatus status =
            g_io_channel_write_chars(channel, new_message, -1, &size, NULL);
        g_free(new_message);
        return status == G_IO_STATUS_NORMAL;
}

void jws_info_set_defaults(JwsInfo *info) {
        g_assert(info);
        JwsInfoPrivate *priv = jws_info_get_instance_private(info);
        priv->rotate_image = TRUE;
        priv->randomize_order = FALSE;
...
JwsWallpaperMode jws_info_get_mode(JwsInfo *info) {
        g_assert(info);
        JwsInfoPrivate *priv;
...
void jws_info_set_mode(JwsInfo *info, JwsWallpaperMode mode) {
        g_assert(info);
        JwsInfoPrivate *priv;
        priv = jws_info_get_instance_private(info);
        priv->mode = mode;
...
gboolean jws_wallpaper_mode_from_info_string(const gchar *mode_string,
                                             JwsWallpaperMode *mode) {
        g_assert(mode);
        g_assert(mode);
